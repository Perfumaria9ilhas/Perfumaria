  ...
  return (
    <div className="min-h-screen bg-[color:var(--sand-soft)]">
      <header className="border-b border-[color:var(--line)] bg-[linear-gradient(180deg,_#ffffff,_#fbf7ef)]">
        <div className="mx-auto flex max-w-7xl flex-col gap-4 px-4 py-5 lg:flex-row lg:items-center lg:justify-between lg:px-6">
          <div>
            ...
            <h1 className="font-serif text-4xl text-[color:var(--ink)]">{title}</h1>
            <p className="text-sm text-slate-600">{description}</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link
              href="/admin"
              className="rounded-full bg-[color:var(--sand-soft)] px-4 py-2 text-sm text-slate-700"
            >
              Dashboard
            </Link>
            <Link
      ...
      <main className="mx-auto max-w-7xl px-4 py-8 lg:px-6">
        <div className="mb-6 flex items-center justify-between rounded-[2rem] border border-[color:var(--line)] bg-white/80 px-5 py-4 shadow-sm">
          <p className="text-sm text-slate-600">
            Alterações no admin refletem-se automaticamente no catálogo público.
          </p>
          <Link
            href="/"
            className="rounded-full bg-[color:var(--sand-soft)] px-4 py-2 text-sm text-[color:var(--ink)]"
          >
            Ver loja
          </Link>
        </div>
